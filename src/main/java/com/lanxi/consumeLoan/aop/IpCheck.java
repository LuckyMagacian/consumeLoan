...
public class IpCheck {
	...
	public String ipCheck(ProceedingJoinPoint point,HttpServletRequest req,HttpServletResponse res){
			...
			LogFactory.debug(this, "用户["+phone+"]非空校验通过!");
			...
			if(cacheIp==null){
				...
				return new RetMessage(RetCodeEnum.FAIL.toString(),"用户尚未登录!",null).toJson();
			}
			...
			if(!cacheIp.equals(ip)){
				...
				return new RetMessage(RetCodeEnum.FAIL.toString(),"用户在其他地点登录!",null).toJson();
			}
			try {
